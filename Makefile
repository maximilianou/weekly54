...
fullstack_web3_5201:
	...
	cd web3-blog && npm install -g yarn
	cd web3-blog && yarn add ethers hardhat @nomiclabs/hardhat-waffle ethereum-waffle chai @nomiclabs/hardhat-ethers web3modal @walletconnect/web3-provider easymde react-markdown react-simplemde-editor ipfs-http-client @emotion/css @openzeppelin/contracts
	cd web3-blog && rm README.md && npx hardhat

fullstack_web3_5202:
	npx hardhat test
	# import metamask first account ( with private key of the local network )
	npx hardhat run scripts/deploy.js

fullstack_web3_5203:	
	npx hardhat node # run test node hardhat localhost
	# metamask first account -> Metamask -> Settings -> [OK] Show test netwroks
	# metamask change network to localhost
	npx run dev # let's run this, with now post at the begining
